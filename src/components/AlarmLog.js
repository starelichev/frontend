...
function AlarmLog() {
  ...
  const fetchEvents = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${API_URL}/api/Event/alarms`, requestData);
      ...
      setEvents([
        {
          id: 1,
          eventCode: 'CRITICAL_001',
          eventName: 'Критическая ошибка',
          date: '2024-04-10T12:55:00',
          objectName: 'КВТ-Юг',
          objectAddress: 'Тульское шоссе, 16',
          deviceName: 'Датчик 676-АЕМ',
          eventDescription: 'Критическая ошибка в работе системы безопасности'
        },
        {
          id: 2,
          eventCode: 'CRITICAL_002',
          eventName: 'Аварийное отключение',
          date: '2024-04-10T12:65:00',
          objectName: 'КВТ-Юг',
          objectAddress: 'Тульское шоссе, 16',
          deviceName: 'Датчик 676-АЕМ',
          eventDescription: 'Аварийное отключение оборудования'
        },
        {
          id: 3,
          eventCode: 'CRITICAL_003',
          eventName: 'Системная ошибка',
          date: '2024-04-10T12:55:00',
          objectName: 'КВТ-Юг',
          objectAddress: 'Тульское шоссе, 16',
          deviceName: 'Датчик 676-АЕМ',
          eventDescription: 'Критическая системная ошибка'
        }
      ]);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="w-[1515px] h-[865px] bg-white rounded-[10px] border border-neutral-400/20 mt-[30px] p-6">
      <div className="flex gap-6 h-full">
        ...
        {/* Right Panel - Alarm Log Table */}
        <div className="flex-1 overflow-hidden flex flex-col">
          <h3 className="text-lg font-semibold mb-4 flex-shrink-0 text-red-600">Аварии</h3>
              ...
              <table className="w-full min-w-[1200px]">
                ...
                <tbody className="divide-y divide-gray-200">
                  {events.map((event, index) => (
                    <tr key={event.id} className={index % 2 === 0 ? 'bg-white' : 'bg-red-50'}>
                      <td className="px-4 py-3 text-sm text-gray-900">№{index + 1}</td>
                      <td className="px-4 py-3 text-sm text-red-600 font-semibold">{event.eventCode}</td>
                      <td className="px-4 py-3 text-sm text-red-600 font-semibold">{event.eventName}</td>
